...
logger = getLog('ant', 'stockcategorylist_gn');
...
def handlePage(n,symbol,browser,timestr):
    ...
    folder_path = fs_config['root'] + "/stockcategorylist_gn/" + symbol
...
def handlePages(symbol,browser,timestr):
    browser.get("http://q.10jqka.com.cn/gn/detail/code/"+symbol+"/") 
    count=0
    try:
...
def query_list():
    _sql = "select code from category where source = 'thsgn'"
    _args = ()
