...
logger = getLog('ant', 'mfinancialindex');
...
def getHtml(url, symbol, timestr): 
    ...
    folder_path = fs_config['root'] + "/mfinancialindex/" + symbol
    ...
    index = j['list'][0];
    if index != None :
        date_time = datetime.datetime.strptime(index['reportdate'],'%Y%m%d')
...
for i in range(len(stocklist)):
    ...
    getHtml("https://xueqiu.com/stock/f10/finmainindex.json?symbol="+symbol+"&page=1&size=2&_=1514872821568", symbol, '2017-09-30')
