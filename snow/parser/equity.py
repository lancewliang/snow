...
fields = [  'reportdate', 'begindate', 'totalshare', 'skchgexp', 'circskamt', 'ncircamt', 'bsk', 'bskchg', 'circbamt', 'cfcircbamt',
          'skchgtype','originatorskamt','pstateamt','pstatecorpamt','pdomecorpamt','pdomesocicorpamt',
          'exrightdate','exrightexp','circskrto','bsharerto' ,'totalbsharerto' 
          ]  
logger = getLog('parser', 'equity');


def getAll(symbol): 
    path = fs_config['root'] + "/equity/" + symbol
...
def getHtml(symbol, fullpath): 
    ...
    if table != None :
        ...
        for i in range(0, len(table)):              
            stock = table[i];        
            if stock ['publishdate']==None:
                stock ['publishdate']= stock ['begindate']
            stock['reportdate'] =stock ['publishdate']     
            dbrs = query_npool(symbol, stock['begindate'])     
            if len(dbrs) == 0:
                    insert(stock, symbol)
                    added = added + 1  
            else:
...
def insert(record, symbol):
    ...
    _sql = "INSERT INTO `snow`.`equity` (`symbol`"
    for f in fields:
        _sql += "," + f;
    _sql += ")VALUES( %s"
    for f in fields:
...
def query_npool(symbol, day): 
    _sql = "select *   from equity where symbol=%s and begindate=%s"
    _args = (symbol, day)
...
def query_list2():
    _sql = "select distinct left(symbol,4) as k from shares where left(symbol,4) in ('SZ30','SH60','SH90','SH00','SZ00','SZ20','SZ39') "
    _args = ()
...
def query_list(v):
    _sql = "select symbol,code from shares where left(symbol,4) ='"+v+"' order by symbol"
    _args = ()
    tasks = query(db_config['local'], _sql, _args)
    return tasks
